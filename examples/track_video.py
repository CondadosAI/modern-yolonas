...
ids, and the fix is a wider expansion or a longer ``--max-lost-seconds``. Running it
twice, once with ``--fusion min``, is the paper's ablation on your own footage.

A lost track is kept for 2 seconds of video by default — in *seconds*, converted with
this clip's frame rate, so the number survives a change of frame rate. ``--keep-all-tracks``
is the paper's setting (never discard anything), which suits a fixed camera on a closed
scene and not much else.
"""
...
def main() -> None:
    ...
    parser.add_argument("--max-lost-seconds", type=float, default=2.0,
                        help="How long a track may go unmatched, in seconds of video. Converted "
                             "to frames with the clip's own frame rate, so the number means the "
                             "same span at 25 fps and at 60.")
    parser.add_argument("--keep-all-tracks", action="store_true",
                        help="Never drop a track — the paper's setting, right for a fixed camera "
                             "on a closed scene and wrong for open-world footage.")
    ...
    tracker = DeepHMSort(
        fusion=args.fusion,
        max_lost_seconds=None if args.keep_all_tracks else args.max_lost_seconds,
    )
